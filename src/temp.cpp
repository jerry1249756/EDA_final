...
void Neighborhood::place_instance_to_each_row(unordered_map<string, instance>& instances, vector<vector<int>>& topdie_row, vector<vector<int>>& bottomdie_row){
    ...
    for(int i=0; i<instances.size(); i++){
        if(instances["C"+to_string(i+1)].part == PART::TOP){ // top
            //if it's very not legal out of range
            if(instances["C"+to_string(i+1)].instance_pos.y>=top_die_upper_y-0.5*top_row_height || instances["C"+to_string(i+1)].instance_pos.y<die_lower_y
            || instances["C"+to_string(i+1)].instance_pos.x>= die_upper_x || instances["C"+to_string(i+1)].instance_pos.x< die_lower_x){ 
                int acceptable_x_max=0;
                int acceptable_y_max=0;
                if(instances["C"+to_string(i+1)].tech == TECH::TECH_A){
                    acceptable_x_max = die_upper_x - tech_stack[0].libcells[instances["C"+to_string(i+1)].libcell_type].width;
                    acceptable_y_max = top_die_upper_y - tech_stack[0].libcells[instances["C"+to_string(i+1)].libcell_type].height;     
                }else if(instances["C"+to_string(i+1)].tech == TECH::TECH_B){
                    acceptable_x_max = die_upper_x - tech_stack[1].libcells[instances["C"+to_string(i+1)].libcell_type].width;
                    acceptable_y_max = top_die_upper_y - tech_stack[1].libcells[instances["C"+to_string(i+1)].libcell_type].height;   
                }
                random_device rd;
                mt19937 gen(rd());
            ...
            //if it's very not legal out of range
            if(instances["C"+to_string(i+1)].instance_pos.y>=bottom_die_upper_y-0.5*bottom_row_height || instances["C"+to_string(i+1)].instance_pos.y<die_lower_y
            || instances["C"+to_string(i+1)].instance_pos.x>= die_upper_x || instances["C"+to_string(i+1)].instance_pos.x< die_lower_x){ 
                int acceptable_x_max=0;
                int acceptable_y_max=0;
                if(instances["C"+to_string(i+1)].tech == TECH::TECH_A){
                    acceptable_x_max = die_upper_x - tech_stack[0].libcells[instances["C"+to_string(i+1)].libcell_type].width;
                    acceptable_y_max = bottom_die_upper_y - tech_stack[0].libcells[instances["C"+to_string(i+1)].libcell_type].height;     
                }else if(instances["C"+to_string(i+1)].tech == TECH::TECH_B){
                    acceptable_x_max = die_upper_x - tech_stack[1].libcells[instances["C"+to_string(i+1)].libcell_type].width;
                    acceptable_y_max = bottom_die_upper_y - tech_stack[1].libcells[instances["C"+to_string(i+1)].libcell_type].height;   
                }
                random_device rd;
                mt19937 gen(rd());
                uniform_int_distribution<> distrib_x(die_lower_x,acceptable_x_max);
...
int Neighborhood::single_row_penalty(vector<vector<int>>& die_row, int row){
    ...
    if(die_row[row].size()>0){
        if(instances["C"+to_string(die_row[row][0])].part == PART::TOP){
...
void Neighborhood::swap_instance(int from, int to){
    int temp_x = find_pos_x(from, instances);
    int temp_y = find_pos_y(from, instances);
    instances["C"+to_string(from)].instance_pos.x = find_pos_x(to, instances);
    instances["C"+to_string(from)].instance_pos.y = find_pos_y(to, instances);
...
void Neighborhood::two_die_swap_instance(int from, int to){
    int temp_x = find_pos_x(from, instances);
    int temp_y = find_pos_y(from, instances);
    TECH temp_tech = find_tech(from, instances);
    PART temp_part = find_part(from, instances);
    instances["C"+to_string(from)].instance_pos.x = find_pos_x(to, instances);
    instances["C"+to_string(from)].instance_pos.y = find_pos_y(to, instances);
    instances["C"+to_string(to)].instance_pos.x = temp_x;
    instances["C"+to_string(to)].instance_pos.y = temp_y;
    instances["C"+to_string(from)].part = find_part(to, instances);
    instances["C"+to_string(to)].part = temp_part;
    instances["C"+to_string(from)].tech = find_tech(to, instances);
    instances["C"+to_string(to)].tech = temp_tech;
}

int Neighborhood::swap_cost(unordered_map<string, instance>& ins, unordered_map<string, net*>& nets, vector<vector<int>>& die_row, int from_row, int from_col, int to_row, int to_col, PART part){
    int cost = 0;
    if(part == PART::TOP){
        for(auto& it : ins["C"+to_string(die_row[from_row][form_col])].connected_nets){
            if("C"+to_string(die_row[from_row][form_col]) != it->top_left ||"C"+to_string(die_row[from_row][form_col]) != it->top_right ||"C"+to_string(die_row[from_row][form_col]) != it->top_up ||"C"+to_string(die_row[from_row][form_col]) != it->top_bottom)
                if(ins["C"+to_string(die_row[to_row][to_col])].instance_pos.x < ins[it->top_left].instance_pos.x){
                    
                }
                ins["C"+to_string(die_row[to_row][to_col])].instance_pos.x > ins[it->top_right].instance_pos.x
                ins["C"+to_string(die_row[to_row][to_col])].instance_pos.y > ins[it->top_up].instance_pos.y
                ins["C"+to_string(die_row[to_row][to_col])].instance_pos.y < ins[it->top_bottom].instance_pos.y){
